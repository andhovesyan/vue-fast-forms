<template>
  <div class="datepicker">
    ...
    <div class="calendar" v-show="isOpen" :class="className" :style="calendarStyle">
      <header>
...
export default {
  ...
  computed: {
    ...
    header() {
      if (this.currView === 'day') {
        return this.currDate.format('MMM YYYY');
      } else if (this.currView === 'month') {
        return this.currDate.format('YYYY');
      } else if (this.currView === 'year') {
        return this.currDate.format('YYYY').replace(/.$/, "0's");;
      }
    },
    emitValue() {
      let value = this.selectedDate.format(this.format);
      return value;
    },
  },
  methods: {
    ...
    toPrev() {
      if (this.currView === 'day') {
        ...
        this.currDate.subtract(1, 'year');
      } else if (this.currView === 'year') {
        this.currDate.subtract(10, 'year');
      }
    },
    toNext() {
      if (this.currView === 'day') {
        ...
        this.currDate.add(1, 'year');
      } else if (this.currView === 'year') {
        this.currDate.add(10, 'year');
      }
    },
    up() {
      if (this.currView === 'day') {
    ...
    selectDate(day) {
      ...
      this.inline ? this.showDayCalendar() : this.close();
    },
    selectMonth(month) {
      if (month.isDisabled) {
        return false;
